...
test('interactive demo reuses the production energy renderer and stays provider neutral', async () => {
  ...
  assert.match(entry, /Continuous/u)
  assert.match(entry, /Reference/u)
  assert.match(entry, /styleVariant=\{style\}/u)
  assert.match(entry, /style === 'reference' \? ratio >= 0\.95 : ratio > 0/u)
  assert.doesNotMatch(entry, /type="color"/u)
...
test('GitHub Pages deploys the built demo and public docs link it', async () => {
  ...
  assert.match(readme, /wsl043\.github\.io\/dsh-native-reasoning-slider/u)
  assert.match(readmeZh, /wsl043\.github\.io\/dsh-native-reasoning-slider/u)
})
